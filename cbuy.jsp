...
rst=stmt.executeQuery("select productname,productprice,quantity,discount,productid,productprice * discount / 100.0 as DiscountAmount,  productprice - (productprice * discount / 100.0)  as discountprice ,gst,units  from product  where productname='"+pname+"' ");
  ...
  <form name="form1" id="form1" action="" method="get">
    ...
    <p align="center">GST is <%= rst.getString(8) %></p>
    ...
    <tr>
      ...
      <td><input name="buyQty" type="text" value="1" /><%=rst.getString(9) %></td>
    </tr>
	<tr>
...
float res=ch/100;
 ...
 // out.print("product  amount"+productAmt);
  ...
  <input type="text" name="storephone"  value="<%=session.getAttribute("user")%>"/>
  <table width="568" border="1" align="center">
    ...
    <tr>
      ...
      <td><p align="right">card holder Name
          <input type="text" name="chname" value="<%=session.getAttribute("customername")%>" />
      </p>
  ...
  <%  
	...
	int x=stmt.executeUpdate("insert into orderlist (productname,productCast,quantity,totalAmount,address,cardHolderName,cardtype,cardNum,cvv,charity,storephone)values('"+t1+"','"+t2+"','"+t3 +"','"+t4+"','"+t5+"','"+t6+"','"+t7+"','"+t8+"','"+t9+"',"+t10+",'"+storephone+"')");
	if(x!=0){
